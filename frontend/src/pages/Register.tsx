...
function Register() {
  const styles = {
    ...
    input_icon: 'absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 pointer-events-none', // fixed
    hide_icon: 'absolute inset-y-0 right-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 cursor-pointer hover:text-text', // fixed
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 400 && errorData?.email) {
          ...
          setError('auth', { type: 'manual', message: status?.toString() });
        }
      } else {
        setError('auth', { type: 'manual', message: 'No se pudo conectar con el servidor. Por favor, verifica tu conexión a internet.' });
      }
    }
  };
  return (
          ...
          <div className={styles.form_container}>
            ...
            <form action="#" className="mt-2 grid grid-cols-6 gap-6" onSubmit={handleSubmit(onSubmit)}>
              <div className={styles.input_container}>
                ...
                <ErrorMessage message={errors.firstname?.message?.toString()} type="field" />
              </div>
              ...
              <div className="relative col-span-6">
                ...
                <ErrorMessage message={errors.email?.message?.toString()} type="field" />
              </div>
              ...
              <div className={styles.input_container}>
                <div className="relative">
                  ...
                  />
                  {showPassword ? 
                    <EyeSlashIcon onClick={togglePasswordVisibility} className={styles.hide_icon} />
                    : <EyeIcon onClick={togglePasswordVisibility} className={styles.hide_icon} />
                  }
                </div>
                <ErrorMessage message={errors.password?.message?.toString()} type="field" />
              </div>
              ...
              <div className={styles.input_container}>
                ...
                <ErrorMessage message={errors.password_confirmation?.message?.toString()} type="field" />
              </div>
              ...
              <ErrorMessage message={errors.auth?.message?.toString()} type="form" />
